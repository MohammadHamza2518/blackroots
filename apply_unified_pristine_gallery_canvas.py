...
new_gallery_container = """          <!-- Product Gallery Viewport Frame (Dynamic Adaptive Luxury Canvas & Zero Cropping) -->
          <div id="ProductMainImageContainer" class="relative w-full aspect-square sm:aspect-[4/3] rounded-3xl overflow-hidden glass-panel-luxury border-2 border-[#d4af37]/40 shadow-2xl flex items-center justify-center group bg-[#0a0c10] p-2 sm:p-4 transition-all duration-300">
            <img id="ProductMainImage" src="./assets/blackroots-bathroom-counter.jpg" alt="BlackRoots Product Showcase" class="max-w-full max-h-full w-auto h-auto object-contain rounded-xl transition-transform duration-300 group-hover:scale-[1.01]">
...
smart_dynamic_canvas_script = """  <script>
    function changeMainProductImage(btn, src) {
      ...
      if (mainImg && imgContainer) {
        ...
        if (src.includes('how-to-use')) {
          imgContainer.className = 'relative w-full aspect-square sm:aspect-[4/3] rounded-3xl overflow-hidden glass-panel-luxury border-2 border-[#d4af37]/40 shadow-2xl flex items-center justify-center group bg-white p-3 sm:p-6 transition-all duration-300';
          mainImg.className = 'max-w-full max-h-full w-auto h-auto object-contain rounded-xl transition-transform duration-300 group-hover:scale-[1.01]';
          ...
          imgContainer.className = 'relative w-full aspect-square sm:aspect-[4/3] rounded-3xl overflow-hidden glass-panel-luxury border-2 border-[#d4af37]/40 shadow-2xl flex items-center justify-center group bg-[#0a0c10] p-2 sm:p-4 transition-all duration-300';
          mainImg.className = 'max-w-full max-h-full w-auto h-auto object-contain rounded-xl transition-transform duration-300 group-hover:scale-[1.01]';
          if (bestsellerBadge) bestsellerBadge.style.display = 'none';
        } else {
          imgContainer.className = 'relative w-full aspect-square sm:aspect-[4/3] rounded-3xl overflow-hidden glass-panel-luxury border-2 border-[#d4af37]/40 shadow-2xl flex items-center justify-center group bg-[#0a0c10] p-2 sm:p-4 transition-all duration-300';
...
for fpath in files:
    if os.path.exists(fpath):
        ...
        if "function changeMainProductImage" in content:
            ...
            if bg_script != -1:
                content = content[:bg_script] + smart_dynamic_canvas_script + "\n</body>\n</html>"

        with open(fpath, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"APPLIED DYNAMIC ADAPTIVE CANVAS ENGINE IN: {fpath}")
