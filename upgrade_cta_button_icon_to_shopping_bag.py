    ...
    if os.path.exists(fpath):
        ...
        if s_idx != -1:
            ...
            if b_start != -1 and b_end != -1:
                content = content[:b_start] + updated_button_html.strip() + content[b_end:]

        with open(fpath, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"UPGRADED CTA BUTTON ICON TO SVG SHOPPING BAG IN: {fpath}")
