...
native_overlay_reels_html = """  <!-- Watch Reels Section (Native Full-Card Transparent Overlay Links to Instagram) -->
  <section class="py-10 sm:py-16 bg-[#07080b] border-b border-[#d4af37]/20 relative overflow-hidden">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
      ...
      <p class="text-gray-300 text-xs sm:text-base max-w-xl mx-auto mb-6 font-light leading-relaxed">
        👈 <strong>Swipe Left & Right</strong> &bull; Click anywhere on video to watch with sound on Instagram 👉
      </p>
        ...
        <div id="ReelsCarouselContainer" class="flex items-center gap-4 sm:gap-6 overflow-x-auto no-scrollbar snap-x snap-mandatory py-4 px-2 sm:px-6 scroll-smooth w-full">
          ...
          <div class="js-reel-card snap-center shrink-0 w-[270px] sm:w-[320px] aspect-[9/16] rounded-3xl overflow-hidden border-2 border-[#d4af37] shadow-2xl relative bg-black group transition-all duration-300 cursor-pointer">
            ...
            <div class="absolute top-3 left-3 right-3 z-30 flex items-center justify-between pointer-events-none gap-2">
              ...
              <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-gradient-to-r from-purple-600 via-pink-600 to-amber-500 text-white font-extrabold text-[9px] sm:text-[10px] tracking-wide shadow-xl border border-white/30 shrink-0">
                📸 Sound on IG 🔊
              </span>
            </div>
...
for fpath in html_files:
    if os.path.exists(fpath):
        ...
        if s_idx != -1:
            ...
            if e_idx != -1:
                content = content[:s_idx] + native_overlay_reels_html + "\n\n  " + content[e_idx+10:]
                with open(fpath, 'w', encoding='utf-8') as f:
                    f.write(content)
                print(f"APPLIED NATIVE OVERLAY LINKS IN: {fpath}")
