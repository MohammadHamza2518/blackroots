...
# 1. REWRITE INFLUENCER PORTAL WITH SHARED LOCALSTORAGE DATABASE (influencer.html)
  ...
  <main id="DashboardSection" class="hidden max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full space-y-8">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div class="p-5 rounded-2xl glass-panel-luxury border border-[#d4af37]/30 bg-[#12151c]/80 shadow-xl space-y-1.5">
        ...
        <div class="text-2xl sm:text-3xl font-extrabold text-white tracking-tight">₹0.00</div>
      ...
      <div class="p-5 rounded-2xl glass-panel-luxury border border-[#d4af37]/30 bg-[#12151c]/80 shadow-xl space-y-1.5">
        ...
        <div class="text-2xl sm:text-3xl font-extrabold text-white tracking-tight">0 Orders</div>
      ...
      <div class="p-5 rounded-2xl glass-panel-luxury border border-[#d4af37]/30 bg-[#12151c]/80 shadow-xl space-y-1.5">
        ...
        <div class="text-2xl sm:text-3xl font-extrabold text-amber-300 tracking-tight">₹0.00</div>
        <p class="text-[11px] text-gray-400 font-medium">₹0 Settled</p>
      </div>
      ...
      <div class="p-5 rounded-2xl glass-panel-luxury border-2 border-emerald-500/40 bg-[#123824]/30 shadow-xl space-y-1.5 relative overflow-hidden">
        ...
        <div class="text-2xl sm:text-3xl font-extrabold text-emerald-400 tracking-tight">₹0.00</div>
        <p class="text-[11px] text-emerald-300 font-semibold">Available for withdrawal</p>
      </div>
  ...
  <div id="PayoutModal" class="fixed inset-0 z-50 bg-black/80 backdrop-blur-md hidden items-center justify-center p-4">
    <div class="bg-[#12151c] border-2 border-[#d4af37]/50 rounded-3xl max-w-md w-full p-6 shadow-2xl relative space-y-5">
      ...
      <div class="space-y-1">
        ...
        <p class="text-xs text-gray-400 font-medium">Enter your UPI ID to receive payment from Store Owner.</p>
      </div>
      ...
      <form onsubmit="handlePayoutSubmit(event)" class="space-y-4 text-xs">
        <div class="space-y-1.5">
          ...
          <input type="text" placeholder="e.g. 9876543210@paytm or name@okicici" required class="w-full bg-[#0a0b0e] border border-white/10 rounded-xl p-3 text-white font-mono focus:outline-none focus:border-[#d4af37]">
        </div>
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      if (loggedUser && db[loggedUser]) {
        renderDashboard(db[loggedUser], loggedUser);
      } else {
        renderLoginGate();
      }
    });
    ...
    function handleLoginSubmit(e) {
      ...
      if (db[userId] && db[userId].pass.trim() === pass) {
        ...
        renderDashboard(db[userId], userId);
...
# 2. REWRITE ADMIN PANEL TO PERSIST ACCOUNTS IN SHARED LOCALSTORAGE (admin-influencer.html)
...
<head>
  ...
  <meta name="description" content="Store Owner Panel for BlackRoots. Assign User IDs, Passwords, track real sales, and settle payouts. Shared database sync.">
...
<body class="bg-[#0a0b0e] text-white font-sans antialiased selection:bg-[#d4af37] selection:text-black min-h-screen flex flex-col justify-between">
  ...
  <header class="sticky top-0 z-50 bg-[#0a0b0e]/95 backdrop-blur-xl border-b border-[#d4af37]/20 py-4 px-4 sm:px-8">
    <div class="max-w-7xl mx-auto flex items-center justify-between">
      <div class="flex items-center gap-3">
        ...
        <span class="text-[10px] font-bold text-emerald-400 bg-emerald-500/20 px-2.5 py-0.5 rounded-full border border-emerald-500/30">SHARED DATABASE ACTIVE</span>
      </div>
  ...
  <script>
    ...
    function handleAddSubmit(e) {
      ...
      const db = getAccountsDB();
      db[userId] = { name, pass, code, rate };
      saveAccountsDB(db);

      renderTables();
      closeAddInfluencerModal();
      showToast(`Influencer ${name} Saved to Database! User ID: ${userId} | Password: ${pass}`);
    }
  </script>
</body>
</html>
...
for d in target_dirs:
    if os.path.exists(d):
        ...
        with open(os.path.join(d, "admin-influencer.html"), "w", encoding="utf-8") as f:
            f.write(admin_html)
        print(f"FIXED SHARED LOCALSTORAGE AUTHENTICATION IN: {d}")
