...
restored_swipe_reels_js = """/* 🎬 Restored Clean Horizontal Swipe Reel Carousel */
function initReelsModal() {
  ...
  // Sound toggle handling for each card
  cards.forEach(card => {
...
for jspath in theme_js_files:
    if os.path.exists(jspath):
        ...
        if r_idx != -1:
            ...
            if e_idx != -1:
                content = content[:r_idx] + restored_swipe_reels_js + "\n\n" + content[e_idx:]
                with open(jspath, 'w', encoding='utf-8') as f:
                    f.write(content)
                print(f"RESTORED SWIPE REELS JS IN: {jspath}")
...
restored_swipe_html = """  <!-- Watch Reels Section (Restored Clean Horizontal Swipe Carousel) -->
        ...
        <div id="ReelsCarouselContainer" class="flex items-center gap-4 sm:gap-6 overflow-x-auto no-scrollbar snap-x snap-mandatory py-4 px-2 sm:px-6 scroll-smooth w-full">
          ...
          <div class="js-reel-card snap-center shrink-0 w-[270px] sm:w-[320px] aspect-[9/16] rounded-3xl overflow-hidden border-2 border-[#d4af37] shadow-2xl relative bg-black group transition-all duration-300">
            <div class="absolute top-3 left-3 right-3 z-30 flex items-center justify-between pointer-events-none">
              ...
              <button type="button" class="js-sound-toggle pointer-events-auto bg-black/80 backdrop-blur-md text-amber-300 text-[10px] font-extrabold px-2.5 py-1 rounded-full border border-amber-500/30 hover:scale-105 transition-transform flex items-center gap-1 shadow-xl cursor-pointer">
...
for fpath in html_files:
    if os.path.exists(fpath):
        ...
        if s_idx != -1:
            ...
            if e_idx != -1:
                content = content[:s_idx] + restored_swipe_html + "\n\n  " + content[e_idx+10:]
                with open(fpath, 'w', encoding='utf-8') as f:
                    f.write(content)
                print(f"RESTORED HORIZONTAL SWIPE REELS SECTION IN: {fpath}")
