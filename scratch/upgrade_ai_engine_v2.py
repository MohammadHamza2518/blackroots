...
ai_engine_code = r"""/* 🩺 Dr. Kuroki — Chief AI Trichologist Clinical Response Engine */
...
# Replace from function initAIConsultantChat to function generateChatGPTStyleHairDoctorResponse or end
start_marker = "/* 🤖 Fluent Hinglish & English Conversational AI Hair Doctor Engine */"
if start_marker not in js_content:
    start_marker = "function initAIConsultantChat()"

start_idx = js_content.find(start_marker)
if start_idx != -1:
    # Find next section or end of file
    end_marker = "/* 📦 Universal Quick Order Modal Controller */"
    end_idx = js_content.find(end_marker, start_idx)
    if end_idx != -1:
        js_content = js_content[:start_idx] + ai_engine_code + "\n\n" + js_content[end_idx:]
    else:
        js_content = js_content[:start_idx] + ai_engine_code
else:
    js_content += "\n" + ai_engine_code
...
print("Updated theme.js with pure AI Engine!")
