...
new_gallery_container = """          <!-- Product Gallery Viewport Frame (Adaptive Aspect Ratio & Zero Cropping & Badge Overlap Protection) -->
          <div class="relative w-full aspect-square sm:aspect-[4/3] rounded-3xl overflow-hidden glass-panel-luxury border-2 border-[#d4af37]/40 shadow-2xl flex items-center justify-center group bg-[#0a0c10] p-2 sm:p-3">
            <img id="ProductMainImage" src="./assets/blackroots-bathroom-counter.jpg" alt="BlackRoots Product Showcase" class="w-full h-full object-contain p-1 sm:p-2 rounded-2xl transition-all duration-300 group-hover:scale-[1.01]">
...
smart_badge_protection_script = """  <script>
    function changeMainProductImage(btn, src) {
      ...
      if (mainImg) {
        ...
        if (src.includes('before-after-infographic') || src.includes('key-ingredients')) {
          mainImg.className = 'w-full h-full object-contain p-1 sm:p-2 rounded-2xl transition-all duration-300 group-hover:scale-[1.01]';
          if (bestsellerBadge) bestsellerBadge.style.display = 'none';
        } else if (src.includes('how-to-use')) {
          mainImg.className = 'w-full h-full object-contain p-3 rounded-2xl bg-white transition-all duration-300 group-hover:scale-[1.01]';
          if (bestsellerBadge) bestsellerBadge.style.display = 'none';
        } else {
          mainImg.className = 'w-full h-full object-contain p-2 rounded-2xl transition-all duration-300 group-hover:scale-[1.01]';
...
for fpath in files:
    if os.path.exists(fpath):
        ...
        if "function changeMainProductImage" in content:
            ...
            if bg_script != -1:
                content = content[:bg_script] + smart_badge_protection_script + "\n</body>\n</html>"

        with open(fpath, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"FIXED ZERO CROPPING & BADGE OVERLAP PROTECTION IN: {fpath}")
