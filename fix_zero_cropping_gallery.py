...
new_gallery_container = """          <div class="relative w-full aspect-square rounded-3xl overflow-hidden glass-panel-luxury border-2 border-[#d4af37]/40 shadow-2xl flex items-center justify-center group bg-[#0a0c10]">
...
zero_crop_script = """  <script>
    function changeMainProductImage(btn, src) {
      const mainImg = document.getElementById('ProductMainImage');
      if (mainImg) {
        mainImg.src = src;
        mainImg.className = 'w-full h-full object-contain p-2 sm:p-3 transition-all duration-300 group-hover:scale-105';
      }
...
for fpath in files:
    if os.path.exists(fpath):
        ...
        start_marker = '<div class="relative w-full aspect-square'
        ...
        if "function changeMainProductImage" in content:
            ...
            if old_script_start != -1 and old_script_end != -1:
                content = content[:old_script_start-12] + zero_crop_script + content[old_script_end+9:]

        with open(fpath, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"FIXED ZERO-CROPPING & FULL VIEWPORT IN: {fpath}")
