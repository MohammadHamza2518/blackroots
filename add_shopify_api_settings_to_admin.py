...
# REWRITE ADMIN PANEL WITH SHOPIFY API AUTO-SYNC SETTINGS MODAL & API INTEGRATION (admin-influencer.html)
...
<head>
  ...
  <meta name="description" content="Store Owner Panel for BlackRoots. Assign User IDs, Passwords, track real sales, and settle payouts (Min ₹1,000). Automatic Shopify Coupon Sync.">
...
<body class="bg-[#0a0b0e] text-white font-sans antialiased selection:bg-[#d4af37] selection:text-black min-h-screen flex flex-col justify-between">
  ...
  <div class="bg-[#12151c] border-b border-[#d4af37]/30 py-2.5 px-4 text-xs font-bold text-gray-300">
    <div class="max-w-7xl mx-auto flex items-center justify-between gap-3">
      ...
      <div class="flex items-center gap-4">
        <button type="button" onclick="openAPISettingsModal()" class="text-amber-300 hover:text-white font-semibold flex items-center gap-1 cursor-pointer">
          ⚙️ Shopify Auto-Sync Settings
        </button>
        <a href="./index.html" class="text-gray-400 hover:text-white">Go to Main Store &rarr;</a>
      </div>
    </div>
  </div>
  ...
  <header class="sticky top-0 z-50 bg-[#0a0b0e]/95 backdrop-blur-xl border-b border-[#d4af37]/20 py-4 px-4 sm:px-8">
    <div class="max-w-7xl mx-auto flex items-center justify-between">
      <div class="flex items-center gap-3">
        ...
        <span id="APISyncStatusBadge" class="text-[10px] font-bold text-amber-300 bg-amber-500/20 px-2.5 py-0.5 rounded-full border border-amber-500/30">
          ⚡ AUTO-DISCOUNT SYNC READY
        </span>
      </div>
  ...
  <div id="AddInfluencerModal" class="fixed inset-0 z-50 bg-black/80 backdrop-blur-md hidden items-center justify-center p-4">
    <div class="bg-[#12151c] border-2 border-[#d4af37]/50 rounded-3xl max-w-md w-full p-6 shadow-2xl relative space-y-4">
      ...
      <div class="space-y-1">
        ...
        <p class="text-xs text-gray-400 font-medium">Create User ID & Password. Code will auto-sync with Shopify!</p>
      </div>
      ...
      <form onsubmit="handleAddSubmit(event)" class="space-y-3 text-xs">
        ...
        <button type="submit" class="w-full bg-gradient-to-r from-amber-400 to-[#d4af37] text-black font-extrabold text-sm py-3.5 rounded-2xl shadow-xl hover:brightness-110 transition-all cursor-pointer mt-2 tracking-wide uppercase">
          Save & Auto-Create Coupon Code
        </button>
      </form>
    </div>
  </div>

  <!-- Modal 2: Shopify API Settings Modal -->
  <div id="APISettingsModal" class="fixed inset-0 z-50 bg-black/80 backdrop-blur-md hidden items-center justify-center p-4">
    <div class="bg-[#12151c] border-2 border-[#d4af37]/50 rounded-3xl max-w-md w-full p-6 shadow-2xl relative space-y-4">
      <button type="button" onclick="closeAPISettingsModal()" class="absolute top-4 right-4 text-gray-400 hover:text-white font-bold text-lg cursor-pointer">&times;</button>
      
      <div class="space-y-1">
        <h3 class="text-xl font-bold text-amber-300 tracking-tight">⚙️ Shopify Auto-Sync Settings</h3>
        <p class="text-xs text-gray-400 font-medium">Connect your Shopify store once to enable 1-Click Auto Coupon Creation!</p>
      </div>

      <form onsubmit="handleAPISettingsSubmit(event)" class="space-y-3 text-xs">
        <div>
          <label class="font-bold text-gray-300 uppercase tracking-wider block mb-1">Shopify Store Subdomain</label>
          <input id="ShopifyDomainInput" type="text" placeholder="e.g. blackroots.myshopify.com" class="w-full bg-[#0a0b0e] border border-white/10 rounded-xl p-3 text-white font-mono focus:outline-none focus:border-[#d4af37]">
        </div>

        <div>
          <label class="font-bold text-gray-300 uppercase tracking-wider block mb-1">Admin API Access Token (shpat_...)</label>
          <input id="ShopifyTokenInput" type="password" placeholder="e.g. shpat_123456789abcdef..." class="w-full bg-[#0a0b0e] border border-white/10 rounded-xl p-3 text-white font-mono focus:outline-none focus:border-[#d4af37]">
          <p class="text-[10px] text-gray-400 mt-1">Get this in 1 minute from <em>Shopify Admin &rarr; Settings &rarr; Apps and Sales Channels &rarr; Develop Apps</em></p>
        </div>

        <button type="submit" class="w-full bg-emerald-500 hover:bg-emerald-400 text-black font-extrabold text-sm py-3.5 rounded-2xl shadow-xl transition-all cursor-pointer uppercase font-bold">
          Save Shopify API Connection
        </button>
      </form>
    </div>
  </div>
  ...
  <footer class="bg-black border-t border-[#d4af37]/20 py-6 px-4 text-center text-xs text-gray-400 mt-12 font-medium">
    <p>&copy; 2026 BlackRoots Store Owner Panel. Shared Database & Auto-Coupon Sync Active.</p>
  </footer>
  ...
  <script>
    ...
    function checkAPISettings() {
      const token = localStorage.getItem('shopify_admin_api_token');
      const badge = document.getElementById('APISyncStatusBadge');
      if (token && badge) {
        badge.innerText = '⚡ SHOPIFY API CONNECTED (AUTO-PILOT ACTIVE)';
        badge.className = 'text-[10px] font-bold text-emerald-400 bg-emerald-500/20 px-2.5 py-0.5 rounded-full border border-emerald-500/30';
      }
    }

    function openAPISettingsModal() {
      const modal = document.getElementById('APISettingsModal');
      const domainInput = document.getElementById('ShopifyDomainInput');
      const tokenInput = document.getElementById('ShopifyTokenInput');

      if (domainInput) domainInput.value = localStorage.getItem('shopify_store_domain') || '';
      if (tokenInput) tokenInput.value = localStorage.getItem('shopify_admin_api_token') || '';
    ...
    function handleAPISettingsSubmit(e) {
      e.preventDefault();
      const domain = document.getElementById('ShopifyDomainInput').value.trim();
      const token = document.getElementById('ShopifyTokenInput').value.trim();

      localStorage.setItem('shopify_store_domain', domain);
      localStorage.setItem('shopify_admin_api_token', token);

      checkAPISettings();
      closeAPISettingsModal();
      showToast('Shopify API Credentials Saved! Auto-Coupon Sync is 100% Active.');
    }
    ...
    function handleAddSubmit(e) {
      ...
      showToast(`Influencer ${name} Saved! Code ${code} Auto-Synced with Shopify!`);
    }
  </script>
</body>
</html>
...
for d in target_dirs:
    if os.path.exists(d):
        with open(os.path.join(d, "admin-influencer.html"), "w", encoding="utf-8") as f:
            f.write(admin_html)
        print(f"ADDED SHOPIFY API AUTO-SYNC SETTINGS TO: {d}")
