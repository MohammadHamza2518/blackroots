...
new_gallery_container = """          <!-- Product Gallery Viewport Frame (Perfect Shrink-Wrap Zero-Gap Zero-Crop Engine) -->
...
perfect_shrink_wrap_script = """  <script>
    function changeMainProductImage(btn, src) {
      ...
      if (mainImg && imgContainer) {
        ...
        if (src.includes('how-to-use')) {
          ...
          mainImg.className = 'w-full h-auto max-h-[550px] object-contain block transition-transform duration-300 group-hover:scale-[1.01]';
...
for fpath in files:
    if os.path.exists(fpath):
        ...
        if "function changeMainProductImage" in content:
            ...
            if bg_script != -1:
                content = content[:bg_script] + perfect_shrink_wrap_script + "\n</body>\n</html>"

        with open(fpath, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"APPLIED PERFECT SHRINK-WRAP FULL-BLEED ENGINE IN: {fpath}")
