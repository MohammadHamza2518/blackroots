...
# 1. REWRITE INFLUENCER PORTAL WITH ₹1,000 MINIMUM PAYOUT ENFORCEMENT & SHARED PAYOUT SYNC
...
<head>
  ...
  <meta name="description" content="BlackRoots Official VIP Creator Login Portal. Access your assigned promo code, track live commission earnings, and request payouts (Min ₹1,000).">
...
# 2. REWRITE ADMIN PANEL TO RENDER AND PROCESS PAYOUTS FROM SHARED DATABASE (admin-influencer.html)
...
<head>
  ...
  <meta name="description" content="Store Owner Panel for BlackRoots. Assign User IDs, Passwords, track real sales, and settle payouts (Min ₹1,000).">
  ...
  <header class="sticky top-0 z-50 bg-[#0a0b0e]/95 backdrop-blur-xl border-b border-[#d4af37]/20 py-4 px-4 sm:px-8">
    <div class="max-w-7xl mx-auto flex items-center justify-between">
      <div class="flex items-center gap-3">
        ...
        <span class="text-[10px] font-bold text-emerald-400 bg-emerald-500/20 px-2.5 py-0.5 rounded-full border border-emerald-500/30">MIN PAYOUT: ₹1,000</span>
      </div>
  ...
  <script>
    ...
    function renderTables() {
      ...
      if (pendingList.length === 0) {
        ...
        payoutsTable.innerHTML = pendingList.map((p, i) => {
...
for d in target_dirs:
    if os.path.exists(d):
        ...
        with open(os.path.join(d, "admin-influencer.html"), "w", encoding="utf-8") as f:
            f.write(admin_html)
        print(f"ENFORCED MIN INR 1000 PAYOUT AND FULL DB SYNC IN: {d}")
