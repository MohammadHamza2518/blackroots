...
new_gallery_container = """          <!-- Product Gallery Viewport Frame (Dynamic Full-Bleed & Perfect Aspect Match Engine) -->
          <div id="ProductMainImageContainer" class="relative w-full rounded-3xl overflow-hidden glass-panel-luxury border-2 border-[#d4af37]/40 shadow-2xl flex items-center justify-center group bg-[#0a0c10] transition-all duration-300">
            <img id="ProductMainImage" src="./assets/blackroots-bathroom-counter.jpg" alt="BlackRoots Product Showcase" class="w-full h-full object-cover transition-transform duration-300 group-hover:scale-[1.01]">
...
perfect_aspect_match_script = """  <script>
    function changeMainProductImage(btn, src) {
      ...
      if (mainImg && imgContainer) {
        ...
        if (src.includes('flatlay-herbs') || src.includes('key-ingredients') || src.includes('before-after-infographic')) {
          // 16:9 Widescreen Images Fill Container 100% Edge-to-Edge With ZERO Gaps & ZERO Cropping
          imgContainer.className = 'relative w-full aspect-[16/9] rounded-3xl overflow-hidden glass-panel-luxury border-2 border-[#d4af37]/40 shadow-2xl flex items-center justify-center group bg-[#0a0c10] p-0 transition-all duration-300';
          mainImg.className = 'w-full h-full object-cover transition-transform duration-300 group-hover:scale-[1.01]';
          ...
          // Bathroom Counter Hero Photo Fits Perfectly
          imgContainer.className = 'relative w-full aspect-[4/3] rounded-3xl overflow-hidden glass-panel-luxury border-2 border-[#d4af37]/40 shadow-2xl flex items-center justify-center group bg-[#0a0c10] p-0 transition-all duration-300';
          mainImg.className = 'w-full h-full object-cover transition-transform duration-300 group-hover:scale-[1.01]';
...
for fpath in files:
    if os.path.exists(fpath):
        ...
        if "function changeMainProductImage" in content:
            ...
            if bg_script != -1:
                content = content[:bg_script] + perfect_aspect_match_script + "\n</body>\n</html>"

        with open(fpath, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"APPLIED PERFECT DYNAMIC ASPECT MATCH ENGINE IN: {fpath}")
