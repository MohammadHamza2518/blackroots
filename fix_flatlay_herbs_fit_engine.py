...
new_gallery_container = """          <!-- Product Gallery Viewport Frame (Aspect Square 1:1 Seamless D2C Fit) -->
          <div class="relative w-full aspect-square rounded-3xl overflow-hidden glass-panel-luxury border-2 border-[#d4af37]/40 shadow-2xl flex items-center justify-center group bg-[#0d0e12] p-1">
...
smart_fit_script = """  <script>
    function changeMainProductImage(btn, src) {
      const mainImg = document.getElementById('ProductMainImage');
      if (mainImg) {
        mainImg.src = src;
        if (src.includes('how-to-use')) {
          mainImg.className = 'w-full h-full object-contain p-3 rounded-2xl bg-white transition-all duration-300 group-hover:scale-[1.02]';
        } else if (src.includes('key-ingredients') || src.includes('before-after-infographic')) {
          mainImg.className = 'w-full h-full object-contain p-2 rounded-2xl transition-all duration-300 group-hover:scale-[1.02]';
        } else {
          mainImg.className = 'w-full h-full object-cover rounded-2xl transition-all duration-300 group-hover:scale-[1.02]';
        }
      }
...
for fpath in files:
    if os.path.exists(fpath):
        ...
        if "function changeMainProductImage" in content:
            ...
            if bg_script != -1:
                content = content[:bg_script] + smart_fit_script + "\n</body>\n</html>"

        with open(fpath, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"FIXED FLATLAY SEAMLESS FULL FIT IN: {fpath}")
