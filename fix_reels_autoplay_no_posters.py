...
autoplay_reels_js = """/* 🎬 Smooth Auto-Play Centered Reel Video Engine (No Picture Posters) */
function initReelsModal() {
  ...
  // IntersectionObserver to detect centered active card on scroll/swipe
  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      ...
      if (entry.isIntersecting) {
        // Highlight active centered video card
        entry.target.classList.add('border-[#d4af37]', 'shadow-[0_25px_65px_rgba(212,175,55,0.45)]', 'opacity-100');
        entry.target.classList.remove('border-white/10', 'opacity-60');
        
        // Instantly play centered video
        const playPromise = video.play();
        if (playPromise !== undefined) {
          playPromise.catch(() => {});
        }
      } else {
        // Pause side off-center video card
        entry.target.classList.remove('border-[#d4af37]', 'shadow-[0_25px_65px_rgba(212,175,55,0.45)]', 'opacity-100');
        entry.target.classList.add('border-white/10', 'opacity-60');
        video.pause();
      }
    });
  }, observerOptions);
...
for jspath in theme_js_files:
    if os.path.exists(jspath):
        ...
        if r_idx != -1:
            ...
            if e_idx != -1:
                content = content[:r_idx] + autoplay_reels_js + "\n\n" + content[e_idx:]
                with open(jspath, 'w', encoding='utf-8') as f:
                    f.write(content)
                print(f"UPGRADED AUTOPLAY REELS JS IN: {jspath}")

# HTML Files Update - REMOVE ALL poster="..." attributes from video tags!
html_files = [
...
no_poster_reels_section_html = """  <!-- Watch Reels Section (Native Smooth Horizontal Swipe & Auto-Play Centered Video) -->
  <section class="py-10 sm:py-16 bg-[#07080b] border-b border-[#d4af37]/20 relative overflow-hidden">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
      ...
      <div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#d4af37]/15 border border-[#d4af37]/40 text-[#d4af37] text-[10px] sm:text-xs font-extrabold uppercase tracking-widest mb-3 backdrop-blur-md shadow-lg">
        <span>🎬 REELS STUDIO &bull; SWIPE TO PLAY</span>
      </div>
      ...
      <p class="text-gray-300 text-xs sm:text-base max-w-xl mx-auto mb-6 font-light leading-relaxed">
        👈 <strong>Swipe Left & Right</strong> to auto-play centered customer transformations 👉
      </p>
      ...
      <div class="relative max-w-6xl mx-auto">
        ...
        <!-- Native Smooth Horizontal Swipe Reel Cards Carousel (No Picture Posters) -->
        <div id="ReelsCarouselContainer" class="flex items-center gap-4 sm:gap-6 overflow-x-auto no-scrollbar snap-x snap-mandatory py-4 px-2 sm:px-6 scroll-smooth w-full">
          ...
          <div class="js-reel-card snap-center shrink-0 w-[270px] sm:w-[320px] aspect-[9/16] rounded-3xl overflow-hidden border-2 border-[#d4af37] shadow-[0_25px_65px_rgba(212,175,55,0.45)] relative bg-black group transition-all duration-300 cursor-pointer">
            ...
            <!-- Pure Video Player (No Picture Poster) -->
            <video autoplay muted playsinline webkit-playsinline loop preload="auto" class="w-full h-full object-cover bg-black">
          ...
          <div class="js-reel-card snap-center shrink-0 w-[270px] sm:w-[320px] aspect-[9/16] rounded-3xl overflow-hidden border-2 border-white/10 opacity-60 shadow-xl relative bg-black group transition-all duration-300 cursor-pointer">
            <div class="absolute top-3 left-3 right-3 z-30 flex items-center justify-between pointer-events-none">
              <span class="bg-black/75 backdrop-blur-md text-amber-300 text-[9px] sm:text-[10px] font-extrabold uppercase px-2.5 py-1 rounded-full border border-amber-500/30 shadow">
                ✨ Anti-Dandruff
              </span>
            ...
            <!-- Pure Video Player (No Picture Poster) -->
            <video muted playsinline webkit-playsinline loop preload="auto" class="w-full h-full object-cover bg-black">
...
for fpath in html_files:
    if os.path.exists(fpath):
        ...
        if s_idx != -1:
            ...
            if e_idx != -1:
                content = content[:s_idx] + no_poster_reels_section_html + "\n\n  " + content[e_idx+10:]
                with open(fpath, 'w', encoding='utf-8') as f:
                    f.write(content)
                print(f"APPLIED NO-POSTER AUTOPLAY REELS SECTION IN: {fpath}")
