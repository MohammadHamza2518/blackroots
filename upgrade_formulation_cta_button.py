...
for fpath in html_files:
    if os.path.exists(fpath):
        ...
        if s_idx != -1:
            ...
            if b_start != -1:
                b_end = content.find('</button>', b_start) + 9
                if b_end != -1:
                    content = content[:b_start] + compact_formulation_button.strip() + content[b_end:]

        with open(fpath, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"UPGRADED FORMULATION CTA BUTTON IN: {fpath}")
