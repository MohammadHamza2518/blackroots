...
clean_html_markup = """        <div class="flex flex-col sm:flex-row gap-3">
          <input type="text" id="HomePincodeInput" maxlength="6" placeholder="Enter 6-Digit Pincode" oninput="window.executePincodeCheck(false)" onkeyup="if(event.key==='Enter') window.executePincodeCheck(true)" class="flex-1 px-5 py-3.5 rounded-2xl bg-black/90 border border-white/20 text-sm text-white font-bold text-center tracking-widest focus:border-[#d4af37] focus:outline-none">
          <button type="button" id="HomeCheckPincodeBtn" onclick="window.executePincodeCheck(true)" class="group relative inline-flex items-center justify-center gap-2 bg-gradient-to-r from-[#d4af37] via-[#f7e7a7] to-[#aa7c11] text-black font-black text-xs sm:text-sm px-6 py-3.5 rounded-2xl border border-[#fff3b0]/70 shadow-lg hover:scale-105 transition-transform cursor-pointer uppercase tracking-wider shrink-0">
    ...
    if os.path.exists(fpath):
        ...
        if s_idx != -1:
            ...
            if b_start != -1 and b_end != -1:
                ...
                if div_end != -1:
                    content = content[:b_start] + clean_html_markup.strip() + "\n\n        " + content[div_end:]

        with open(fpath, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"CLEANED PINCODE HTML IN: {fpath}")

# 2. Update theme.js files with simple bulletproof function
theme_js_files = [
...
clean_js = """/* 🚚 Ultra-Clean Bulletproof Pincode Engine */
window.executePincodeCheck = function(isExplicitClick) {
  ...
  let days = 3;
  if (raw.startsWith('20') || raw.startsWith('11') || raw.startsWith('40') || raw.startsWith('70') || raw.startsWith('56') || raw.startsWith('50')) {
    days = 2;
  }
...
for jspath in theme_js_files:
    if os.path.exists(jspath):
        ...
        if p_idx == -1:
            p_idx = content.find('/* 🚚 Ultra-Clean Bulletproof')
        if p_idx == -1:
            p_idx = content.find('window.calculateDeliveryDateStr')

        if p_idx != -1:
            content = content[:p_idx] + clean_js
        else:
            content += "\n\n" + clean_js

        with open(jspath, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"CLEANED PINCODE JS ENGINE IN: {jspath}")
