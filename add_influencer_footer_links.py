...
for fpath in files:
    if os.path.exists(fpath):
        ...
        if "influencer.html" not in content and "</footer>" in content:
            content = content.replace("</footer>", f"{footer_links_snippet}\n</footer>")
            with open(fpath, 'w', encoding='utf-8') as f:
                f.write(content)
            print(f"ADDED INFLUENCER & ADMIN LINKS TO FOOTER IN: {fpath}")
