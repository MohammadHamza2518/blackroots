...
sleek_sound_icon_reels_html = """  <!-- Watch Reels Section (Native Full-Card Transparent Overlay Links to Instagram & Sleek Sound Icon) -->
  <section class="py-10 sm:py-16 bg-[#07080b] border-b border-[#d4af37]/20 relative overflow-hidden">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
      ...
      <p class="text-gray-300 text-xs sm:text-base max-w-xl mx-auto mb-6 font-light leading-relaxed">
        👈 <strong>Swipe Left & Right</strong> &bull; Click video or sound icon to watch with full audio on Instagram 👉
      </p>
      ...
      <div class="relative max-w-6xl mx-auto">
        ...
        <div id="ReelsCarouselContainer" class="flex items-center gap-4 sm:gap-6 overflow-x-auto no-scrollbar snap-x snap-mandatory py-4 px-2 sm:px-6 scroll-smooth w-full">
          ...
          <div class="js-reel-card snap-center shrink-0 w-[270px] sm:w-[320px] aspect-[9/16] rounded-3xl overflow-hidden border-2 border-[#d4af37] shadow-2xl relative bg-black group transition-all duration-300 cursor-pointer">
            ...
            <div class="absolute top-3.5 left-3.5 right-3.5 z-30 flex items-center justify-between pointer-events-none gap-2">
              ...
              <span class="w-8 h-8 rounded-full bg-black/80 backdrop-blur-md text-amber-300 border border-amber-500/40 flex items-center justify-center text-xs shadow-xl shrink-0" title="Watch on Instagram with Sound">
                🔊
              </span>
            </div>
            
            <video autoplay muted loop playsinline webkit-playsinline preload="auto" class="w-full h-full object-cover bg-black pointer-events-none">
              <source src="./assets/reel-3.mp4" type="video/mp4">
            </video>
            
            <div class="absolute bottom-3.5 left-3.5 right-3.5 z-30 p-2.5 rounded-2xl bg-black/85 backdrop-blur-xl border border-[#d4af37]/40 flex items-center justify-between gap-2 shadow-2xl">
              ...
              <a href="product.html" class="js-trigger-order relative z-40 bg-gradient-to-r from-[#d4af37] to-[#aa7c11] text-black text-[9px] font-black px-3 py-1.5 rounded-xl uppercase tracking-wider shrink-0 shadow-lg hover:scale-105 transition-transform pointer-events-auto">
                Buy Now &rarr;
              </a>
            </div>
          </div>
          ...
          <div class="js-reel-card snap-center shrink-0 w-[270px] sm:w-[320px] aspect-[9/16] rounded-3xl overflow-hidden border-2 border-[#d4af37]/60 shadow-xl relative bg-black group transition-all duration-300 cursor-pointer">
...
for fpath in html_files:
    if os.path.exists(fpath):
        ...
        if s_idx != -1:
            ...
            if e_idx != -1:
                content = content[:s_idx] + sleek_sound_icon_reels_html + "\n\n  " + content[e_idx+10:]
                with open(fpath, 'w', encoding='utf-8') as f:
                    f.write(content)
                print(f"APPLIED SLEEK COMPACT SOUND ICON IN: {fpath}")
