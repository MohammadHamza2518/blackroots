...
swipeable_reels_js = """/* 🚀 Native Swipeable Reels Carousel (Instagram / YouTube Shorts Style) */
function initReelsModal() {
        ...
        if (video.muted) {
          ...
          cards.forEach(c => {
            ...
            if (btn && ico) {
              ...
              btn.classList.add('bg-black/70', 'text-amber-300');
            }
          });
          video.muted = false;
          soundIcon.textContent = '🔊';
          soundBtn.classList.remove('bg-black/70', 'text-amber-300');
  ...
  // Intersection Observer to auto-play visible centered card on swipe
  ...
  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      ...
      if (entry.isIntersecting) {
        entry.target.classList.add('border-[#d4af37]', 'shadow-[0_20px_60px_rgba(212,175,55,0.4)]');
        entry.target.classList.remove('border-white/10');
        const playPromise = video.play();
        if (playPromise !== undefined) {
          playPromise.catch(() => {});
        }
      } else {
        entry.target.classList.remove('border-[#d4af37]', 'shadow-[0_20px_60px_rgba(212,175,55,0.4)]');
        entry.target.classList.add('border-white/10');
        video.pause();
      }
    });
  }, observerOptions);
...
for jspath in theme_js_files:
    if os.path.exists(jspath):
        ...
        if r_idx != -1:
            ...
            if e_idx != -1:
                content = content[:r_idx] + swipeable_reels_js + "\n\n" + content[e_idx:]
                with open(jspath, 'w', encoding='utf-8') as f:
                    f.write(content)
                print(f"UPGRADED SWIPEABLE REELS JS IN: {jspath}")
...
swipeable_reels_section_html = """  <!-- Watch Reels Carousel Section (Instagram / TikTok Swipeable Reels) -->
  <section class="py-10 sm:py-16 bg-[#07080b] border-b border-[#d4af37]/20 relative overflow-hidden">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
      ...
      <div class="relative max-w-6xl mx-auto">
        ...
        <!-- Left Arrow Button -->
        <button id="ReelsSlideLeft" type="button" class="hidden sm:flex absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 z-40 w-11 h-11 rounded-full bg-black/80 border border-[#d4af37] text-[#d4af37] hover:bg-[#d4af37] hover:text-black items-center justify-center font-bold text-lg shadow-2xl transition-all cursor-pointer focus:outline-none">
          &larr;
        </button>

        <!-- Right Arrow Button -->
        <button id="ReelsSlideRight" type="button" class="hidden sm:flex absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 z-40 w-11 h-11 rounded-full bg-black/80 border border-[#d4af37] text-[#d4af37] hover:bg-[#d4af37] hover:text-black items-center justify-center font-bold text-lg shadow-2xl transition-all cursor-pointer focus:outline-none">
          &rarr;
        </button>

        <!-- Native Swipeable Reel Cards Carousel -->
        <div id="ReelsCarouselContainer" class="flex items-center gap-4 sm:gap-6 overflow-x-auto no-scrollbar snap-x snap-mandatory py-4 px-2 sm:px-6 scroll-smooth w-full">
          ...
          <!-- Reel Card 1 (Fix Grey Hair, Dandruff, Fall) -->
          <div class="js-reel-card snap-center shrink-0 w-[270px] sm:w-[320px] aspect-[9/16] rounded-3xl overflow-hidden border-2 border-[#d4af37] shadow-[0_20px_60px_rgba(212,175,55,0.4)] relative bg-black group transition-all duration-300">
          ...
          <!-- Reel Card 2 (Say No To Flaky Dandruff) -->
          <div class="js-reel-card snap-center shrink-0 w-[270px] sm:w-[320px] aspect-[9/16] rounded-3xl overflow-hidden border-2 border-white/10 shadow-xl relative bg-black group transition-all duration-300">
            <!-- Top Controls Overlay -->
            <div class="absolute top-3 left-3 right-3 z-30 flex items-center justify-between pointer-events-none">
              <span class="bg-black/75 backdrop-blur-md text-amber-300 text-[9px] sm:text-[10px] font-extrabold uppercase px-2.5 py-1 rounded-full border border-amber-500/30 shadow">
                ✨ Anti-Dandruff
          ...
          <!-- Reel Card 3 (Results Are 100% Real) -->
          <div class="js-reel-card snap-center shrink-0 w-[270px] sm:w-[320px] aspect-[9/16] rounded-3xl overflow-hidden border-2 border-white/10 shadow-xl relative bg-black group transition-all duration-300">
            <!-- Top Controls Overlay -->
            <div class="absolute top-3 left-3 right-3 z-30 flex items-center justify-between pointer-events-none">
              <span class="bg-black/75 backdrop-blur-md text-amber-300 text-[9px] sm:text-[10px] font-extrabold uppercase px-2.5 py-1 rounded-full border border-amber-500/30 shadow">
          ...
          <!-- Reel Card 4 (Stop Premature Greying) -->
          <div class="js-reel-card snap-center shrink-0 w-[270px] sm:w-[320px] aspect-[9/16] rounded-3xl overflow-hidden border-2 border-white/10 shadow-xl relative bg-black group transition-all duration-300">
            <!-- Top Controls Overlay -->
            <div class="absolute top-3 left-3 right-3 z-30 flex items-center justify-between pointer-events-none">
              <span class="bg-black/75 backdrop-blur-md text-amber-300 text-[9px] sm:text-[10px] font-extrabold uppercase px-2.5 py-1 rounded-full border border-amber-500/30 shadow">
          ...
          <!-- Reel Card 5 (Your Roots, Reborn Black) -->
          <div class="js-reel-card snap-center shrink-0 w-[270px] sm:w-[320px] aspect-[9/16] rounded-3xl overflow-hidden border-2 border-white/10 shadow-xl relative bg-black group transition-all duration-300">
            <!-- Top Controls Overlay -->
            <div class="absolute top-3 left-3 right-3 z-30 flex items-center justify-between pointer-events-none">
              <span class="bg-black/75 backdrop-blur-md text-amber-300 text-[9px] sm:text-[10px] font-extrabold uppercase px-2.5 py-1 rounded-full border border-amber-500/30 shadow">
...
for fpath in html_files:
    if os.path.exists(fpath):
        ...
        if s_idx != -1:
            ...
            if e_idx != -1:
                content = content[:s_idx] + swipeable_reels_section_html + "\n\n  " + content[e_idx+10:]
                with open(fpath, 'w', encoding='utf-8') as f:
                    f.write(content)
                print(f"UPGRADED SWIPEABLE REELS CAROUSEL IN: {fpath}")
