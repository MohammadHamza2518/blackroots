...
def update_reviews_file(file_path):
    ...
    # Remove existing Sunita and Alok cards if present
    if 'id="sunita-verma"' in content or 'id="alok-mishra"' in content:
        # Regex to strip old sunita and alok divs
        content = re.sub(r'<!--\s*(?:#1|#2|\s*)\s*Hero Review:\s*Sunita Verma.*?</div>\s*</div>', '', content, flags=re.DOTALL)
        content = re.sub(r'<!--\s*(?:#1|#2|\s*)\s*Hero Review:\s*Alok Mishra.*?</div>\s*</div>', '', content, flags=re.DOTALL)
        content = re.sub(r'<div id="sunita-verma".*?</div>\s*</div>', '', content, flags=re.DOTALL)
        content = re.sub(r'<div id="alok-mishra".*?</div>\s*</div>', '', content, flags=re.DOTALL)

    # Insert Sunita Verma (#1) and Alok Mishra (#2) at top of review grid
    grid_marker = '<div class="columns-1 md:columns-2 lg:columns-3 gap-6 space-y-6">'
    if grid_marker in content and 'id="sunita-verma"' not in content:
        content = content.replace(grid_marker, grid_marker + '\n' + sunita_hero_card + '\n' + alok_hero_card)

    # Update applySort function to preserve Pinned Hero Reviews at the very top
    old_apply_sort = """    function applySort(order) {
    ...
    new_apply_sort = """    function applySort(order) {
      const cards = getVisibleCards();
      const sorted = cards.sort((a, b) => {
        const aPinned = a.id === 'sunita-verma' || a.id === 'alok-mishra';
    ...
    if old_apply_sort in content:
        content = content.replace(old_apply_sort, new_apply_sort)

    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(content)
    print(f"Pinned Sunita Verma & Alok Mishra as #1 & #2 Hero Reviews in {file_path}!")
...
print("HERO REVIEWS PINNING COMPLETE!")
